...
def myorder(request):
	...
	for i in carts:
		net_price=net_price+i.total_price
	return render(request,'myorder.html',{'carts':carts,'net_price':net_price})

def seller_myorder(request):
	carts=[]
	seller=User.objects.get(email=request.session['email'])
	carts1=Cart.objects.filter(payment_status=True)
	for i in carts1:
		if i.product.seller==seller:
			carts.append(i)
	return render(request,'seller-myorder.html',{'carts':carts})